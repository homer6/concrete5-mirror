...
<div id="newImg">
	...
	<select name="playback" style="vertical-align: middle">
		<option value="ORDER"<?php  if ($playback == 'ORDER') { ?> selected<?php  } ?>>Display Order</option>
		<option value="RANDOM-SET"<?php  if ($playback == 'RANDOM-SET') { ?> selected<?php  } ?>>Random (But keep sets together)</option>
		<option value="RANDOM"<?php  if ($playback == 'RANDOM') { ?> selected<?php  } ?>>Completely Random</option>
	</select>
	</td>
	<td style="text-align: right">
	<?php echo $ah->button_js('Add Image', 'SlideshowBlock.chooseImg()');?>
	</td>
	</tr>
	</table>
</div>
