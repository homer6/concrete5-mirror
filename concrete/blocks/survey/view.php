...
if ($controller->hasVoted()) { ?>
	...
	<div style="">		
		...
		<strong>Question: <?php echo $controller->getQuestion()?></strong>
		...
		<div id="surveyResults" style=" <?php echo (!$_GET['dontGraphPoll'] && $totalVotes>0)?'float:left; width:45%':''?>">
			...
			<div class="note" style="margin-top:8px"><?php echo intval($totalVotes)?> Vote<?php echo (intval($totalVotes)==1)?'':'s'?></div>
	...
	<?php  if($_GET['dontGraphPoll']){ ?>
		<div class="small right" style="margin-top:8px"><a class="arrow" href="<?php echo DIR_REL?>/?cID=<?php echo $b->getBlockCollectionID() ?>">View Full Results</a></div>
	<?php  } ?>
