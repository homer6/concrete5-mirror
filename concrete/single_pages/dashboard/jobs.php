	...
	<table class="grid-list" width="100%" cellspacing="1" cellpadding="0" border="0">
		...
		<tr id="jobItemRow<?php echo $jobItem['jID']?>" 
			...
			<td class="center">
				...
				<form method="post" action="<?php echo $this->url('/dashboard/jobs', 'uninstall')?>" onsubmit="return Jobs.confirmUninstall();">
...
<?php  if (count($availableJobs) > 0) { ?>
	...
	<table class="grid-list" width="100%" cellspacing="1" cellpadding="0">
		...
		<tr> 
			<td><?php echo $availableJobObj->jName ?></td>
			<td><?php echo $availableJobObj->jDescription ?></td> 
			<td class="center">
				<?php  if(!$availableJobObj->invalid){ ?>
				<form method="post" action="<?php echo $this->url('/dashboard/jobs', 'install')?>">
